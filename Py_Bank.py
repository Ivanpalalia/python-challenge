...
with open(file_path, 'r') as file_handle:
    ...
    #start by getting first row values for initial calculation
    row_one = next(csv_handle)

    #then, count variable to get total number of months
    count = 1

    #now,initializing variables to calculate average profit/loss and month with greatest profit and loss
    avg_sum = 0
    total = int(row_one[1])
    start_profit = 0
    start_loss = 0
    ...
    #loop through by going over all rows one at a time
    for row in csv_handle:
        count = count + 1
        #first getting profit/loss difference between previous and current month
        profit_loss = int(row[1]) - int(start_num)
        #then, adding profit/loss to get total amount of profit and loss difference
        avg_sum = avg_sum + profit_loss
        total = total + int(row[1])
        #now, comparing the profit/loss to find month with greatest profit and loss difference
        ...
        if profit_loss < start_loss:
            ...
            loss_mth = row[0]
       #now, resetting the starting number for next row caluclation
        start_num = row[1]
